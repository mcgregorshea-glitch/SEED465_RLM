...
from utils import PRINTER_BOUNDS  # Single source of truth for hardware limits
...
class GCodeSenderGUI:
    """
    Main application class for the G-Code Sender GUI.
    Provides a comprehensive control interface for 3D printers/CNC machines,
    including real-time position tracking, manual jogging, G-code file streaming,
    and integrated measurement data collection from DMMs.
    """
    ...
    def __init__(self, parent_frame):
        """
        Initializes the G-Code Sender GUI and its internal state.
        
        Args:
            parent_frame: The Tkinter parent widget (usually the main notebook frame).
        """
        ...
        # --- Color Palette ---
        self.COLOR_BG = "#0a0e14" # Deep background color
        self.COLOR_PANEL_BG = "#161b22" # Secondary background for panels
        self.COLOR_BORDER = "#30363d" # Border color for widgets
        self.COLOR_TEXT_PRIMARY = "#e6edf3" # Main text color
        self.COLOR_TEXT_SECONDARY = "#7d8590" # Muted text color
        self.COLOR_ACCENT_CYAN = "#00d4ff" # Primary accent color (Status/Active)
        self.COLOR_ACCENT_GREEN = "#3fb950" # Success status color
        self.COLOR_ACCENT_AMBER = "#ffa657" # Warning/Caution status color
        self.COLOR_PENDING_RING = "#c4c1ff" # Border color for idle/pending states
        self.COLOR_ACCENT_RED = "#ff4444" # Critical error/Stop status color
        self.COLOR_BLACK = "#000000" # Terminal/Canvas background
        self.COLOR_GREY_COMPLETED = "#484f58" # Color for finished path segments
        ...
        style.configure('PurpleRing.TButton', background=self.COLOR_PANEL_BG, foreground=self.COLOR_TEXT_PRIMARY, bordercolor=self.COLOR_PENDING_RING, borderwidth=1, relief=tk.SOLID, padding=(12, 8), font=self.FONT_BODY)
        style.map('PurpleRing.TButton', background=[('active', '#2c333e'), ('pressed', self.COLOR_BLACK)], foreground=[('active', self.COLOR_PENDING_RING)], bordercolor=[('active', self.COLOR_PENDING_RING)])
        ...
        style.configure('TEntry',
                        ...
                        bordercolor=self.COLOR_BORDER,
                        insertcolor=self.COLOR_ACCENT_CYAN, 
                        borderwidth=1,
                        relief=tk.SOLID,
                        padding=6,
                        font=self.FONT_MONO)
        ...
        self.is_disconnecting = False           # Flag to prevent race conditions during disconnect
        self.homing_performed = False           # Track if homing has been performed
        self.hardware_fault = False             # Flag indicating a hardware communication error
        ...
        self.rotation_crash_test_complete = False # Safety flag required before scan execution
        
        # Threading events for controlling background tasks
        ...
        self.pause_event.set()                  # Initialized in the non-paused state
        ...
        self.history_index = 0                  # Current index for history navigation
        
        # Data structures for visualizing the G-code toolpath
        self.toolpath_by_layer: Dict[float, List[Any]] = {}   # Map of Z-levels to segments
        self.move_to_layer_map: List[Any] = []   # Mapping of global move index to layer info
        self.ordered_z_values: List[float] = []    # List of Z-levels in appearance order
        self.completed_move_count: int = 0      # Progress tracking for visualization
        self.after_id: Any = None                # ID for the main polling 'after' job
        ...
        self._plot_cache_valid = False          # Validation flag for the 3D cache

        # Control variable for enabling/disabling plots for performance
        self.is_3d_plot_enabled = tk.BooleanVar(value=True) # UI toggle for 3D view
        self._3d_control_bar: Optional[ttk.Frame] = None    # Control bar for 3D tab
        self.is_2d_plot_enabled = tk.BooleanVar(value=True) # UI toggle for 2D canvases

        # --- Printer Physical Bounds (in mm) ---
        # Sourced from utils.py — edit limits there to affect both panels.
        self.PRINTER_BOUNDS: Dict[str, float] = dict(PRINTER_BOUNDS)
        ...
        self.port_var = tk.StringVar(value=self.available_ports[0] if self.available_ports else "") # Port selection
        ...
        # Progress and visualization variables
        ...
        self.last_cmd_abs_x: Optional[float] = None             # Last known X coordinate (abs)
        self.last_cmd_abs_y: Optional[float] = None             # Last known Y coordinate (abs)
        self.last_cmd_abs_z: Optional[float] = None             # Last known Z coordinate (abs)
        ...
        # Top-level layout: Header (TOP), Footer (BOTTOM), and Main View (CENTER/EXPAND)
        self.create_header_bar(self.parent)
        self.create_footer_bar(self.parent)
        ...
        self.marker_3d = None                   # Artist for the red position dot
        ...
        self.create_control_frame(self.left_panel_scrollable)        # EXECUTION CONTROL
    ...
    def create_header_bar(self, parent):
        """Creates the top header bar with title, status indicator, and help button."""
        # Use a fixed height for the header to ensure it's never clipped to 0
        header_bar = ttk.Frame(parent, style='Header.TFrame', height=50)
        header_bar.pack(side=tk.TOP, fill=tk.X)
        header_bar.pack_propagate(False) # Force the frame to keep its defined height

        # --- Pack elements in EXACT order to ensure space allocation ---
        
        # 1. First: header_status_indicator
        self.header_status_indicator = StatusIndicator(header_bar, self.COLOR_PANEL_BG)
        self.header_status_indicator.pack(side=tk.RIGHT, padx=(5, 20), pady=10)

        # 2. Second: info_button
        # NOTE: padx right=95 clears the 80px E-STOP canvas overlay placed at top-right of root window
        self.info_button = tk.Button(
            header_bar, text="?", font=("Segoe UI", 12, "bold"),
            bg=self.COLOR_PANEL_BG, fg=self.COLOR_ACCENT_CYAN,
            activebackground=self.COLOR_PANEL_BG, activeforeground="#ffffff",
        ...
        # 3. Third: title_label
        title_label = ttk.Label(header_bar, text="⚡ G-CODE SENDER", style='Header.TLabel', 
                                font=self.FONT_HEADER, foreground=self.COLOR_ACCENT_CYAN)
        title_label.pack(side=tk.LEFT, padx=15)

        # 4. Fourth: file_label
        file_label = ttk.Label(header_bar, textvariable=self.header_file_var, style='Header.TLabel', 
                               font=self.FONT_MONO, foreground=self.COLOR_TEXT_SECONDARY)
        file_label.pack(side=tk.LEFT, expand=True, fill=tk.X)

    # _show_tutorial_popup is defined further below (full implementation).
    ...
    def create_connection_frame(self, parent):
        ...
        self.conn_home_button = ttk.Button(frame, text="⌂", style='PurpleRing.TButton', command=self._home_all, width=3)
        self.conn_home_button.grid(row=0, column=6, padx=(0, 10), sticky="w")
        
        # Shift the cancel button to column 7
        self.cancel_connect_button = ttk.Button(frame, text="Cancel", command=self._cancel_connection_attempt)
        self.cancel_connect_button.grid(row=0, column=7, sticky="w", padx=(0, 10))
    ...
    def create_measurement_frame(self, parent):
        """
        Creates the 'MEASUREMENT' panel for DMM integration and logging settings.
        
        Args:
            parent: The parent widget to pack the measurement frame into.
        """
        ...
        self.browse_log_btn = ttk.Button(mid_frame, text="Save data as...", command=self.select_log_file, style='YellowRing.TButton')
    ...
    def create_control_frame(self, parent):
        """
        Creates the 'EXECUTION CONTROL' panel with file picker, job controls, and progress tracking.
        
        Args:
            parent: The parent widget to pack the control frame into.
        """
    ...
    def _toggle_2d_plot_button(self):
        """Toggles the visibility state of the 2D toolpath canvases."""
    ...
    def _update_2d_plot_button_style(self):
        """Updates the visual style of the 2D toggle button based on its active state."""
    ...
    def create_position_control_frame(self, parent):
        ...
        Args:
            parent: The parent widget to pack the position control frame into.
        """
    ...
    def create_manual_control_frame(self, parent):
        """
        Creates the 'MANUAL JOG CONTROL' panel for interactive printer movement.
        
        Args:
            parent: The parent widget to pack the manual control frame into.
        """
    ...
    def create_log_panel(self, parent):
        ...
        Args:
            parent: The parent widget (tab) to pack the log panel into.
        """
    ...
    def create_view_controls(self, parent):
        """
        Creates a compact horizontal toolbar for 3D view controls (rotation and presets).
        
        Args:
            parent: The parent widget to pack the view controls into.
        """
    ...
    def _create_3d_plot_widgets(self, parent):
        """
        Internal method to instantiate matplotlib widgets for the 3D toolpath visualization.
        
        Args:
            parent: The parent widget to pack the plot into.
        """
        plot_frame = ttk.Frame(parent, style="Panel.TFrame")
        plot_frame.pack(fill=tk.BOTH, expand=True)

        def deferred_load():
            """The actual import and creation logic, run after a short delay to keep UI snappy."""
            try:
    ...
    def _update_3d_plot_visibility(self):
        """Refreshes the 3D plot tab content based on whether the visualization is enabled."""
    ...
    def create_3d_display_panel(self, parent):
        """
        Creates the '3D View' notebook tab with control bar and plot container.
        
        Args:
            parent: The parent widget (tab frame) to populate.
        """
        parent.columnconfigure(0, weight=1)
        ...
        self.dmm_reading_label = tk.Label(
            control_bar,
            textvariable=self.last_measurement_var,
            font=self.FONT_MONO,
            foreground=self.COLOR_ACCENT_CYAN,
            background=self.COLOR_PANEL_BG,
            padx=15
        )
        self.dmm_reading_label.pack(side=tk.LEFT, padx=(20, 0))
    ...
    def _toggle_3d_plot_button(self):
        """Toggles the enabled/disabled state of the 3D toolpath plot for performance tuning."""
        self.is_3d_plot_enabled.set(not self.is_3d_plot_enabled.get())
        self._update_3d_plot_button_style()
        self._update_3d_plot_visibility()

    def _update_3d_plot_button_style(self):
        """Updates the visual style of the 3D toggle button based on its active state."""
        if self.is_3d_plot_enabled.get():
            self.toggle_3d_button.config(style='Custom.Toggle.On.TButton')
        else:
            self.toggle_3d_button.config(style='Custom.Toggle.Off.TButton')

    def _style_3d_plot(self, elev=None, azim=None):
        """
        Applies a custom dark theme and viewing angle to the matplotlib 3D plot.
    ...
    def _are_collinear(self, p1, p2, p3, epsilon=1e-6):
        """
        Determines if three 3D points are collinear using the cross-product method.
        Used to simplify the visualized toolpath by removing redundant intermediate points.
        
        Args:
            p1, p2, p3 (tuple): (X, Y, Z) coordinate tuples.
            epsilon (float): Threshold for numerical precision.
            
        Returns:
            bool: True if points are approximately collinear.
        """
    ...
    def _build_plot_coordinates(self):
        """
        Processes the raw toolpath data into simplified vertex arrays for plotting.
        Applies collinearity reduction to improve matplotlib rendering performance.
        
        Returns:
            tuple: (x_array, y_array, z_array) of coordinates.
        """
    ...
    def _is_3d_tab_active(self):
        """
        Checks if the 3D View tab is currently selected.
        
        Returns:
            bool: True if the 3D View tab is active.
        """
        try:
    ...
    def _on_tab_changed(self, event=None):
        """Triggered when the user switches tabs. Refreshes the 3D plot if it has become visible."""
    ...
    def _draw_3d_toolpath(self):
        """
        Renders the full simplified G-code toolpath on the 3D axes.
        Handles view angle preservation and conditional rendering based on tab visibility.
        """
        ...
        # Get current viewing angle so the plot doesn't "snap" back to iso view.
        curr_elev = self.ax_3d.elev
        curr_azim = self.ax_3d.azim

        self.ax_3d.clear()
        self.marker_3d = None # Clear the reference to the old marker artist
        self._style_3d_plot(elev=curr_elev, azim=curr_azim) # Re-apply style after clearing
    ...
    def _update_3d_position_marker(self):
        """Updates the red marker on the 3D plot to match the printer's last known physical coordinates."""
        if not self.is_3d_plot_enabled.get() or not self.matplotlib_imported or not self.ax_3d:
            return
        if not self._is_3d_tab_active():
            return  # Skip render when 3D tab is hidden
    ...
    def _set_view(self, elev, azim):
        """
        Sets the viewing orientation of the 3D plot.
    ...
    def _rotate_view(self, elev_change=0, azim_change=0):
        """
        Adjusts the 3D plot viewing angle by relative increments.
        
        Args:
            elev_change (float): Change in elevation angle.
            azim_change (float): Change in azimuth angle.
        """
        if not self.matplotlib_imported or not self.ax_3d:
            return
    ...
    def _color_blend(self, color1_hex, color2_hex, alpha):
        """
        Blends two hexadecimal colors by a specified alpha ratio.
        
        Args:
            color1_hex, color2_hex (str): Hex color strings.
            alpha (float): Blend ratio (0.0 to 1.0).
    ...
    def _get_available_ports(self):
        ...
        Filters for common USB-to-Serial bridges typically found on 3D printer controller boards.
        
        Returns:
            list: A list of device path strings for available ports.
        """
    ...
    def rescan_ports(self):
        """
        Updates the connection dropdown with the latest list of detected serial ports.
        Maintains the user's current selection if it remains available.
        """
    ...
    def _set_manual_controls_state(self, state):
        """
        Changes the interactive state (enabled/disabled) of the manual jog and homing widgets.

        Args:
            state (str): The Tkinter state constant ('normal' or 'disabled').
        """
    ...
    def _set_goto_controls_state(self, state):
        """
        Changes the interactive state of the 'Go To' coordinate input fields and canvases.
    ...
    def _set_terminal_controls_state(self, state):
        """
        Enables or disables the manual command terminal entry and dispatch button.
    ...
    def select_file(self):
        ...
        if filepath:
            ...
            self.log_filepath_var.set(os.path.join("Data Files", f"DATA_FROM_{base_name}.csv"))

        # Enable the start button if we are connected and have a valid file.        if self.serial_connection and self.processed_gcode:
            self.start_button.config(state=tk.NORMAL)

    def clear_file(self):
        """Clears the currently loaded G-code program and stops any active job."""
        if self.is_sending or self.is_manual_command_running:
            self.queue_message("Aborting active execution to clear file.", "WARN")
            self.stop_event.set()
            self.pause_event.set() # Unblock sender thread if it was paused
        ...
        # Update UI Styles back to Purple (Lighter purple)
        if hasattr(self, '_ctrl_frame'):
            self._ctrl_frame.config(style='Purple.TLabelframe')
        if hasattr(self, 'select_gcode_button'):
            self.select_gcode_button.config(style='PurpleRing.TButton')
        ...
        # Refresh all 2D displays (XY, Z, E) to clear the cyan lines
        self._update_all_displays()
    ...
    def load_gcode_file(self, filepath):
        ...
        try:
            ...
            # Reset the crash test status as this is a new file
            self.rotation_crash_test_complete = False
    ...
    def process_gcode(self):
        ...
        with open(self.gcode_filepath, 'r') as f:
            for line in f:
                line_number += 1
                # Strip comments for processing logic
    ...
    def disconnect_printer(self, silent=False):
        ...
        # Drain ALL status-related messages from the queue to prevent stale updates
        # from re-asserting an "on" or "busy" state after we've disconnected.
        try:
            ...
            while True:
                try:
                    ...
                    # Messages that should NOT be processed after a disconnect:
                    stale_types = ('CONNECTED', 'SET_STATUS', 'POSITION_UPDATE', 
                                 'PROGRESS', 'PATH_PROGRESS_UPDATE', 'MANUAL_FINISHED')
                    
                    if item[0] not in stale_types:
            ...
            if drained_count > 0:
                self.log_message(f"[DIAG] Drained {drained_count} stale status messages.", "WARN")
                
            # Put back the messages we want to keep (like LOGs)
            for item in preserved:
                self.message_queue.put(item)
        except Exception as e:
            self.log_message(f"[DIAG] Queue drain error: {e}", "ERROR")
        ...
        # Use .config() for ttk widgets — consistent with the working connect path
        self.connect_button.config(text="Connect", style='PurpleRing.TButton')
        self.connect_button.state(['!disabled'])
        if hasattr(self, '_conn_frame'):
            self._conn_frame.config(style='Purple.TLabelframe')
        self.port_combobox.config(state="readonly")
                    ...
                    try:
                        if self.serial_connection.in_waiting > 0:
                            ...
                            while '\n' in response_buffer:
                                ...
                                if full_line:
                                    if 'ok' not in full_line.lower():
                                        self.queue_message(f"Received: {full_line}")
                                        
                                        # --- Limit switch automatic polling ---
                                        if 'endstops hit' in full_line.lower() or 'endstop hit' in full_line.lower():
                                            self.queue_message("Unexpected limit switch hit detected! Polling location...", "WARN")
                                            with self.serial_lock:
                                                self.serial_connection.write(b'M114\n')
                                                
                                        # --- Parse M114 inline response if present ---
                                        if 'X:' in full_line and 'Y:' in full_line and 'Z:' in full_line:
                                            x_match = re.search(r'X:\s*([-+]?\d*\.?\d+)', full_line)
                                            y_match = re.search(r'Y:\s*([-+]?\d*\.?\d+)', full_line)
                                            z_match = re.search(r'Z:\s*([-+]?\d*\.?\d+)', full_line)
                                            e_match = re.search(r'E:\s*([-+]?\d*\.?\d+)', full_line)
                                            
                                            if x_match and y_match and z_match:
                                                pos_update = {'x': float(x_match.group(1)), 'y': float(y_match.group(1)), 'z': float(z_match.group(1))}
                                                if e_match:
                                                    pos_update['e'] = float(e_match.group(1))
                                                self.message_queue.put(("POSITION_UPDATE", pos_update))
                                                current_target.update(pos_update)
                                    else:
                                        # Still log ok for manual commands, but don't parse it
                                        self.queue_message(f"Received: {full_line}")
    ...
    def gcode_sender_thread(self, gcode_to_send):
        ...
        def wait_for_ok(timeout_s=30):
            start_t = time.time()
            buf = ""
            while time.time() - start_t < timeout_s:
                if self.stop_event.is_set(): raise InterruptedError("Stop")
                if self.serial_connection.in_waiting > 0:
                    buf += self.serial_connection.read(self.serial_connection.in_waiting).decode('utf-8', errors='ignore')
                    if 'ok' in buf.lower(): return True
                time.sleep(0.05)
            return False

        while cmd_index < len(gcode_to_send):
            line = gcode_to_send[cmd_index]
            ...
            if is_move_command:
                ...
                if parsed:
                    ...
                    # We skip the very first layer change (moves_sent_in_thread == 0) since no drift could have occurred yet.
                    # We also skip if this is the very first Z move of the scan (moving to Layer 1).
                    if target_pos_for_line['z'] != last_pos['z'] and last_pos['z'] is not None:
                        is_first_z_move = (last_pos['z'] == self.PRINTER_BOUNDS['z_min'])

                        if moves_sent_in_thread > 0 and not is_first_z_move:
                            # Only update the layer start index if we aren't currently retrying
                            if cmd_index != layer_start_index:
                                layer_start_index = cmd_index
                                layer_start_moves = moves_sent_in_thread
                                # Note: We don't reset layer_retry_count here yet; we wait for a SUCCESSFUL verification
                                # to prove the previous layer was completed without needing more retries.

                            self.queue_message(f"Layer change detected ({last_pos['z']:.2f} -> {target_pos_for_line['z']:.2f}). Running homing verification...")
                        try:
                            self._homing_verification_routine(auto_restart=True)
                            # SUCCESS: The previous layer moves (and the Z transition) were accurate.
                            ...
                            # --- Automatic Speed Reduction Logic ---
                            # If we hit 3 drifts in a row on the same layer boundary, we are likely moving too fast.
                            if layer_retry_count >= 3:
                                current_cap = getattr(self, '_active_speed_cap', 99999)
                                if current_cap > 1000:
                                    new_cap = 1000
                                else:
                                    new_cap = current_cap / 2
                                
                                self._active_speed_cap = new_cap
                                self.queue_message(f"STABILITY ALERT: 3 consecutive drifts detected. Reducing max travel speed to {new_cap:.0f} mm/min.", "WARN")
                                layer_retry_count = 0 # Reset counter after applying the penalty to allow 3 more tries at the new speed
                                
                            self.queue_message(f"DRIFT DETECTED (Attempt {layer_retry_count}/3): {diag}", "WARN")
                            self.queue_message("Attempting recovery: Re-homing X/Y and restarting layer...", "INFO")
                            
                            try:
                                # Physically re-home to fix the drift and wait for 'ok' to clear buffer
                                with self.serial_lock:
                                    self.serial_connection.write(b'G28 X Y\n')
                                wait_for_ok(timeout_s=60)
                                with self.serial_lock:
                                    self.serial_connection.write(b'M400\n')
                                wait_for_ok(timeout_s=30)
                                
                                # Roll back state
                                cmd_index = layer_start_index
                                moves_sent_in_thread = layer_start_moves
                                # Update UI progress bars to show the rollback
                                self.message_queue.put(("PATH_PROGRESS_UPDATE", moves_sent_in_thread))
                                continue # Jump to start of loop with reset index
                            except Exception as recovery_err:
                                self.queue_message(f"Recovery failed: {recovery_err}", "CRITICAL")
                                self._handle_homing_failure(diag)
            ...
            if 'X' in gcode_line.upper() and 'Y' in gcode_line.upper() and 'Z' not in gcode_line.upper():
                if sent_line_count > 1: # Let first initial home pass through normally
                    ...
                    try:
                        ...
                        # Similar restart logic for explicit G28 XY
                        diag = str(e)
                        self.queue_message(f"DRIFT DETECTED: {diag}", "WARN")
                        self.queue_message("Attempting recovery: Re-homing X/Y and restarting layer...", "INFO")
                        try:
                            with self.serial_lock:
                                self.serial_connection.write(b'G28 X Y\nM400\n')
            ...
            try:
                ...
                # Use our helper to wait for 'ok'
                timeout_val = 60.0 # Blanket 60s timeout for all commands
                wait_for_ok_result = wait_for_ok(timeout_s=timeout_val)

                if wait_for_ok_result:
                    ...
                    # If this was a move (G0/G1) and auto-measure is ON, take a reading.
                    if is_move_command and self.is_dmm_connected and self.auto_measure_enabled.get():
                        # 1. Send M400 to ensure the printer has physically reached the coordinate
                        with self.serial_lock:
                            self.serial_connection.write(b'M400\n')
                        
                        # Wait for the M400 to finish (up to 60s)
                        if wait_for_ok(timeout_s=60):
                            self.queue_message("Auto-Measure: Measuring...")
                            # 2. Trigger Measurement with Stability Check
                            if self.dmm_group:
                                try:
                                    # This method handles pre-delay, triggering, and reading
                ...
                if not wait_for_ok_result:
                    self.queue_message(f"Warning: No 'ok' received for '{gcode_line}' (timeout: {timeout_val:.1f}s).", "WARN")
                    # If we didn't get an 'ok', we usually want to abort to prevent safety issues.
                    success = False
                    break
        ...
        try:
            while True: # Process all messages currently in the queue.
                ...
                if msg_type == "LOG":
                    ...
                    if self.serial_connection is None: continue
                    ...
                    if (self.serial_connection is None or self.is_disconnecting) and msg_content not in ("off", "error"):
                        self.log_message(f"Ignored stale SET_STATUS '{msg_content}' while disconnected or disconnecting.", "DEBUG")
                        continue
                    ...
                    if self.is_disconnecting:
                        self.log_message("Ignored stale CONNECTED message during disconnect.", "WARN")
                        # We must close the connection if it was just established in the background
                        new_conn = msg_content[0]
                        if new_conn:
                            try: new_conn.close()
                            except: pass
                        continue
    ...
    def _show_scan_complete_popup(self):
        """
        Displays a prominent modal popup to notify the user that the scan has
        completed successfully. Far more visible than a log message alone.
        """
        ...
        # --- Style constants (reuse app palette) ---
        BG       = getattr(self, 'COLOR_PANEL_BG', '#0d1b2a')
        FG_HEAD  = '#00e676'          # vivid green
        FG_SUB   = getattr(self, 'COLOR_TEXT_SECONDARY', '#8899aa')
        FG_BTN   = '#ffffff'
        BTN_BG   = '#1b5e20'          # dark green button
        BTN_HOV  = '#2e7d32'

        popup.configure(bg=BG, highlightthickness=3, highlightbackground=FG_HEAD)

        # --- Icon row ---
        icon_frame = tk.Frame(popup, bg=BG)
        icon_frame.pack(pady=(30, 0), padx=50)
        ...
        tk.Label(
            popup,
            text="The G-code stream finished successfully.\nAll commands were acknowledged by the printer.",
            font=("Segoe UI", 11),
            fg=FG_SUB,
            bg=BG,
            justify="center",
        ...
        # --- Line count summary ---
        total = getattr(self, 'total_lines_to_send', 0)
        if total:
            tk.Label(
                popup,
                text=f"{total} lines sent",
        ...
        btn.pack(pady=(0, 30))
        btn.bind("<Return>", lambda _e: _close())
    ...
    def _show_collision_test_complete_popup(self):
        """
        Displays a prominent modal popup to notify the user that the collision
        test has completed successfully.
        """
        popup = tk.Toplevel(self.root)
        popup.title("Collision Test Complete")
        popup.resizable(False, False)
        popup.grab_set()  # Make it modal
        popup.attributes("-topmost", True)
        ...
        tk.Label(
            popup,
            text="The collision avoidance test finished successfully.\nThe printer has returned to the center position.",
            font=("Segoe UI", 11),
            fg=FG_SUB,
            bg=BG,
            justify="center",
    ...
    def _show_tutorial_popup(self):
        """
        Displays a comprehensive modal tutorial popup by reading instructions
        from the HINTS_TUTORIAL.txt file.
        """
        popup = tk.Toplevel(self.root)
        popup.title("SEED Control Center - Tutorial & Guide")
        ...
        tk.Label(
            header, 
            text="ⓘ  MACHINE OPERATING GUIDE", 
            font=("Segoe UI", 16, "bold"),
            fg=ACCENT,
            bg=BG_DARK
        ).pack(side=tk.LEFT, padx=20)

        # --- Content Area (Standard Text with Scrollbar) ---
        ...
        # --- Load text from file (path resolved relative to this script) ---
        ...
        if os.path.exists(tutorial_file):
            try:
                ...
                txt_area.insert(tk.END, f"Error loading tutorial file: {e}")
        else:
            txt_area.insert(tk.END, f"Tutorial file not found at:\n{tutorial_file}")

        txt_area.configure(state=tk.DISABLED) # Read only
        ...
        btn = tk.Button(
            ...
            bg=self.COLOR_ACCENT_GREEN if hasattr(self, 'COLOR_ACCENT_GREEN') else "#00e676",
            fg="#000000",
    ...
    def _update_section_borders(self):
        ...
        self.log_message(f"[DIAG] State: connected={bool(self.serial_connection)}, homed={self.homing_performed}", "DEBUG")

        # CONNECTION — green once printer is connected
        if hasattr(self, '_conn_frame'):
            if self.serial_connection is None:
                self._conn_frame.config(style=PURPLE)
            elif not self.homing_performed:
                self._conn_frame.config(style=YELLOW)
            else:
                self._conn_frame.config(style=GREEN)

            if hasattr(self, 'connect_button'):
                if self.serial_connection is None:
                    self.connect_button.config(text="Connect", style='PurpleRing.TButton')
                else:
                    style = 'GreenRing.TButton' if self.homing_performed else 'YellowRing.TButton'
                    self.connect_button.config(text="Disconnect", style=style)
            
            # Dynamic Homing Button Styling
            if hasattr(self, 'conn_home_button'):
                homing_done = getattr(self, 'homing_performed', False)
                self.conn_home_button.config(style='GreenRing.TButton' if homing_done else 'PurpleRing.TButton')
        ...
        try:
            with open(filepath, 'a', newline='', encoding='utf-8') as f:
                ...
                if not file_exists:
                    ...
                    # Use actual connected DMM names for the header columns
                    if self.dmm_group and self.dmm_group.dmms:
                        dmm_headers = [f"{d.name} ({mode})" for d in self.dmm_group.dmms]
                    else:
                        dmm_headers = [f"Reading ({mode})"]
                    
                    headers = ["Timestamp", "X", "Y", "Z", "Rotation"] + dmm_headers
    ...
    def _collision_test_worker(self):
        ...
        try:
            ...
            # --- 0. Pre-Flight ---
            # Brief delay to allow UI to catch up and clear any stale serial data
            time.sleep(1.0)
            if self.serial_connection:
                with self.serial_lock:
                    self.serial_connection.reset_input_buffer()
            ...
            try:
                ...
                if hasattr(self, 'lbl_test_status') and self.lbl_test_status.winfo_exists():
                    self.root.after(0, lambda: self.lbl_test_status.config(
                        text="Test Failed: Center coordinates not marked.", 
            ...
            # Step 2.4: Return Tilt (E) to 0 before homing any axis
            update_status("Resetting tilt axis to 0°...")
            cmd = self._apply_e_conversion(f"G90\nG1 E0 F{speed_e}\nM400")
            ...
            if self.serial_connection:
                ...
                self.root.after(0, lambda: self.lbl_test_status.config(text="Test Complete", fg=self.COLOR_TEXT_PRIMARY))
            ...
            if hasattr(self, 'lbl_test_status') and self.lbl_test_status.winfo_exists():
                self.root.after(0, lambda e=e: self.lbl_test_status.config(
