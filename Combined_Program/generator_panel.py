...
class PatternGeneratorGUI:
    ...
    def __init__(self, parent_frame):
        ...
        # ===== MENU BAR REMOVED =====
        # Menu features (Load Profile) are now embedded in the UI or main application.
    ...
    def create_input_panel(self, parent):
        ...
        # Callback for when the user wants to send the generated G-code
    ...
    def draw_preview_diagram(self, params, bounds_warnings, warning_level=0):
        ...
        # --- NEW: Draw Rotation Preview ---
        rot_r = 35 # radius
        rot_cx = canvas_w - rot_r - 20 # 20px padding from right
        rot_cy = canvas_h - rot_r - 20 # 20px padding from bottom
        
        # Background arc (forbidden zone top, allowed zone bottom)
        ...
        if params is not None and 'rot_min' in params and 'rot_max' in params:
            rmin = params['rot_min']
            rmax = params['rot_max']
            # Clamp for preview visuals
            rmin = max(-90, min(90, rmin))
            rmax = max(-90, min(90, rmax))
            
            # Correct mapping: tk_angle = 270 + e_deg
            start_angle = 270 + rmin
            extent_angle = rmax - rmin
            
            # Draw the arc for the scan range
            self.preview_canvas.create_arc(
                rot_cx - rot_r, rot_cy - rot_r, rot_cx + rot_r, rot_cy + rot_r,
                start=start_angle, extent=extent_angle, fill=self.COLOR_ACCENT_PURPLE, outline=self.COLOR_ACCENT_CYAN, style=tk.PIESLICE
            )
    ...
    def _start_generation_process(self, send_to_sender=False):
        ...
        if format_choice == "gcode":
            ...
            if not fname: return
            self._generate_gcode_file(params, total_points, fname, send_to_sender=False)
    ...
    def create_gcode(self, pattern_generator, params, total_points):
        ...
        for i, pos in enumerate(pattern_generator, 1):
            if first_point:
                yield "; --- Initial Safe Move to Scan Center (Z first) ---"
                yield f"G1 Z{pos['z']:.3f} F{params['travelspeed']:.0f} ; Move Z to min Z"
                yield f"G1 X{pos['x']:.3f} Y{pos['y']:.3f} E{pos['rotation']:.3f} F{params['travelspeed']:.0f} ; Move X, Y, E to start pos"
            ...
            if last_z is not None and pos['z'] != last_z:
                yield "; --- End of Layer Verification & Rehoming ---"
                yield "G28 X Y ; Sync and verify X/Y"
                yield f"G1 Z{pos['z']:.3f} F{params['travelspeed']:.0f} ; Ensure Z is correct after rehome"
                yield ""
            last_z = pos['z']
